    ...
    if (products.status === 200 && products.success) {
        ...
        for (const productId in purchaseProductList) {
            ...
            if (product) {
                ...
                const productToAdd = { productid: productId, label: product.productName, price, quantity, totalPrice, discountPercentage };
...
const addProductToTable = (product) => {
    const { label: productName, price: productPrice, productid: productId, quantity: productQuantity, discountPercentage } = product;
...
window.paymentButton = async () => {
    ...
    if (Object.keys(purchaseProductList).length > 0 && idExistingSupplier && idExistingSupplier.id) {
        ...
        const purchaseItem = {
            supplierID: idExistingSupplier.id,
